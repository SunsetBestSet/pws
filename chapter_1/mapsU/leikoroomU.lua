...
function leiko_room:manageKeypresses(key, GAME)
	if key == 'space' and self.interact_leiko then 
		...
		Talkies.say("Leiko", "I don't exactly know what happens in detail except for this line here, “for the clouds and weakest will cry first”. Whatever that meeans.", {image=self.leiko_avatar, talkSound=GAME.blop})
		Talkies.say("Hiko", "Is that why there were crazy clouds around the mountain?!", {image=GAME.player.avatar, talkSound=GAME.blop})
		Talkies.say("Leiko", "Well... I might be. If that really is where the disk was sealed. Do you thinks that's what you saw?", {image=self.leiko_avatar, talkSound=GAME.blop})
